...
def _md_pass_groups(passes_on: list[str], axes: dict) -> str:
    """Passing workflows for a markdown cell, grouped by the value tuple across
    all the failure axes (e.g. `gpu AMD, api D3D12: 2: A, B`), else a flat list."""
    if not passes_on:
        return "-"
    groups = group_passes_by_axes(passes_on, axes)
    if groups is None:
        return _md_wf_list(passes_on)
    dims = failure_axis_dims(axes)
    def head(key):
        return ", ".join(f"{d} {v}" for d, v in zip(dims, key))
    return "<br>".join(f"**{head(key)}** — {_md_wf_list(wfs)}" for key, wfs in groups)
...
def attribute_divergence(fails_on: list[str], passes_on: list[str]) -> dict:
    ...
    *runtime* behaviour, which legitimately varies per configuration (a driver
    ...
    passes) it IS upgraded to `compiler_suspected_*` by
    `divergence_suspect_prefix`. That clean partition is exactly the
    high-confidence case: the same compiler is wrong on every configuration that
    runs it while the other compiler is right, which points at the compiler
...
def same_compiler_passes(per_wf: dict[str, str], compiler: str) -> bool:
    """True if some workflow using `compiler` (clang / dxc) PASSED this test.

    Used to gate compiler blame: a same-compiler pass proves that compiler can
    build/run the test, so a failure on another same-compiler workflow is
    environment/toolchain-version specific, not a compiler defect. Absence of any
    such pass is the high-confidence signal that the compiler is consistently at
    fault.
    """
    return any(res == "PASS" and parse_workflow_axes(w).get("compiler") == compiler
               for w, res in per_wf.items())




# Order failure-axis dimensions most-specific-first when several align at once.
_AXIS_PRIORITY = ("gpu", "api", "compiler", "host", "variant")


def failure_axis_dims(axes: dict) -> list[str]:
    """The failure-axis dimensions present (gpu / api / compiler / host /
    variant), most-specific-first. Empty when the failing set aligns on no axis.
    When several are present the failure is the *intersection* of those values
    (e.g. Vulkan AND AMD), and the pass side is grouped by that same set.
    """
    present = {k[:-len("_pattern")] for k in axes if k.endswith("_pattern")}
    return [d for d in _AXIS_PRIORITY if d in present]


def group_passes_by_axes(passes_on: list[str], axes: dict
                         ) -> list[tuple[tuple[str, ...], list[str]]] | None:
    """Group the passing workflows by their value tuple across *all* the failure
    axes, so the pass side reads as a direct contrast to the (homogeneous)
    failing combination on the same axes — e.g. fails on `gpu: AMD-only,
    api: Vulkan-only`, passes grouped into (AMD, D3D12) / (NVIDIA, Vulkan) / …
    The tuple is ordered to match `failure_axis_dims(axes)`. Returns an ordered
    [(value_tuple, [workflows])] list (largest group first), or None when there
    is no failure axis to group on — the caller then lists all passes flat.
    """
    dims = failure_axis_dims(axes)
    if not dims or not passes_on:
        return None
    groups: dict[tuple[str, ...], list[str]] = {}
    for w in passes_on:
        wa = parse_workflow_axes(w)
        key = tuple(wa.get(d) or "unknown" for d in dims)
        groups.setdefault(key, []).append(w)
    return [(k, sorted(ws))
            for k, ws in sorted(groups.items(), key=lambda kv: (-len(kv[1]), kv[0]))]
...
# The prefix names the tightest aligning layer: `runtime_driver_suspected`
# (per-vendor GPU split), `api_backend_suspected` (clean API-axis split), or
# `compiler_suspected` (clean compiler split — every workflow on that compiler
# fails, the other compiler passes); see CLASSIFICATION_LEGEND and
# `divergence_suspect_prefix`.
...
def divergence_suspect_prefix(axes: dict) -> str:
    ...
    attributed axes. The offload-test-suite exists to exercise the clang and dxc
    compilers, so we DO blame a compiler when we can say so with high confidence
    — but never when the fault is really the backend or driver.
    ...
    Priority (most specific first):
      ...
      * api-aligned  -> `api_backend_suspected` (API/backend): same binary,
        splits cleanly on the API with no per-vendor alignment.
      * compiler-aligned -> `compiler_suspected`: the failing set is exactly one
        compiler AND no workflow using that compiler passes the test AND at
        least one workflow using the OTHER compiler passes. `attribute_divergence`
        only emits `compiler_pattern` under that clean partition, which is what
        makes the blame high-confidence: if the failure were a backend/driver
        issue, the same compiler would pass on a different backend (a
        same-compiler pass), and no compiler_pattern would be produced. The
        passing other-compiler proves the test/golden is valid, so the wrong
...
def _label_color(label: str) -> str:
    ...
    if "shader_compile" in l:
        # De-blamed variant (a same-compiler peer compiled it): mute it so it
        # doesn't read as a compiler fault.
        return "#57606a" if "env_suspected" in l else "#953800"
    if l == "build_failure":
        return "#cf222e"
    if l == "xpass":
        return "#0969da"
...
def _html_wf_list(names: list[str], annotate: dict[str, str] | None = None,
                  run_urls: dict[str, str] | None = None) -> str:
    ...
    failure mode) as a small muted suffix. When `run_urls` maps the name, the
    pill links to that workflow's CI run."""
    if not names:
        return '<span class="muted">\u2014</span>'
    pills = []
    for n in names:
        ...
        url = run_urls.get(n) if run_urls else None
        if url:
            pills.append(f'<a class=wf href="{html.escape(url)}" target=_blank>{label}</a>')
        else:
...
def _html_pass_groups(passes_on: list[str], axes: dict,
                      run_urls: dict[str, str] | None = None) -> str:
    """Render the passing workflows, grouped by their value tuple across all the
    failure axes (each group headed by one axis-value chip per axis), else a flat
    pill list. Each pill links to that workflow's CI run."""
    if not passes_on:
        return '<span class="muted">\u2014</span>'
    groups = group_passes_by_axes(passes_on, axes)
    if groups is None:
        return _html_wf_list(passes_on, run_urls=run_urls)
    dims = failure_axis_dims(axes)
    return "".join(
        f'<div class=passgrp>'
        + "".join(_html_axis_chip(d, v) for d, v in zip(dims, key))
        + f' {_html_wf_list(wfs, run_urls=run_urls)}</div>'
        for key, wfs in groups
    )
...
def render_html_report(run_ts: str, summary: list[dict], divergences: list[dict],
    ...
    # fails-on / passes-on pills link to each workflow's CI run.
    run_urls = {r["workflow"]: r["run_url"] for r in summary if r.get("run_url")}
    if divergences:
