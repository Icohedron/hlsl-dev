...
in
{
  ...
  devcontainer.settings = {
    ...
    # Auto-activation for every terminal, and for the shell an editor runs to
    # capture the project environment -- which is how clangd and the Python
    # tooling end up seeing this toolchain rather than the bare image. The
    # workspace, the submodules and any `wt` worktree are owned by a foreign
    # uid, hence safe.directory.
    postCreateCommand = ''
      set -eu
      grep -qs 'devenv hook' ~/.bashrc || printf '\neval "$(devenv hook bash)"\n' >>~/.bashrc
      grep -qs bashrc ~/.profile || printf '\n[ -n "$BASH_VERSION" ] && [ -f ~/.bashrc ] && . ~/.bashrc\n' >>~/.profile
